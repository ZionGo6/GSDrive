...
collision_gamma = 0.99  # Increased for longer-horizon credit assignment
distance_deviation_gamma = 0.99
angle_deviation_gamma = 0.99
gae_lambda = 0.95
angle_deviation_threshold = 40
static_collision_score_threshold = 25000

REWARD_SCALE = 10.0
PROGRESS_REWARD_SCALE = 0.1 * REWARD_SCALE
...
def compute_advantages_and_metrics(clip_data, d_max):
    ...
    dynamic_collision_lastgaelam = 0.0
    static_collision_lastgaelam = 0.0
    distance_deviation_lastgaelam = 0.0
    angle_deviation_lastgaelam = 0.0
    next_dynamic_collision_value = 0.0
    next_static_collision_value = 0.0
    next_distance_deviation_value = 0.0
    next_angle_deviation_value = 0.0
    ...
    for ts in clip_inverse:
        ...
        if not step_info['game_over']:
            if next_distance_deviation is None or next_is_dynamic_collision is None or next_angle_deviation is None or next_is_static_collision is None:
                ...
                progress_reward = 0.0  # Initialize progress reward in first iteration
                ...
                dynamic_collision_reward = (-1.0 if next_is_dynamic_collision else 0.1) * REWARD_SCALE
                static_collision_reward = (-1.0 if next_is_static_collision else 0.1) * REWARD_SCALE

                if next_distance_deviation > d_max:
                    distance_deviation_reward = -1.0 * REWARD_SCALE
                elif next_distance_deviation < 1.0:
                    distance_deviation_reward = 0.2 * REWARD_SCALE
                else:
                    distance_deviation_reward = 0.0
           
                if next_distance_deviation > 0.2 and abs(next_angle_deviation) > angle_deviation_threshold:
                    angle_deviation_reward = -1.0 * REWARD_SCALE
                elif abs(next_angle_deviation) < 10:
                    angle_deviation_reward = 0.1 * REWARD_SCALE
                else:
                    angle_deviation_reward = 0.0
                
                if prev_distance_deviation is not None:
                    progress = prev_distance_deviation - next_distance_deviation
                    progress_reward = progress * PROGRESS_REWARD_SCALE
                else:
                    progress_reward = 0.0
                prev_distance_deviation = next_distance_deviation

            rl_value = step_info.get('rl_value_function', 0.0)
            if rl_value is None or np.isnan(rl_value) or np.isinf(rl_value):
                rl_value = 0.0
            rl_value = float(np.clip(rl_value, -10.0, 10.0))

            dynamic_collision_delta = dynamic_collision_reward + collision_gamma * next_dynamic_collision_value - rl_value
            ...
            static_collision_delta = static_collision_reward + collision_gamma * next_static_collision_value - 0 * rl_value
            ...
            distance_deviation_delta = distance_deviation_reward + distance_deviation_gamma * next_distance_deviation_value - 0 * rl_value
            ...
            angle_deviation_delta = angle_deviation_reward + angle_deviation_gamma * next_angle_deviation_value - 0 * rl_value
            ...
            progress_delta = progress_reward + distance_deviation_gamma * 0.0 - 0 * rl_value
            progress_advantage = progress_delta  

            def sanitize_advantage(adv):
                if np.isnan(adv) or np.isinf(adv):
                    return 0.0
                return float(np.clip(adv, -100.0, 100.0))

            next_dynamic_collision_value = rl_value
            next_static_collision_value = 0.0
            next_distance_deviation_value = 0.0
            next_angle_deviation_value = 0.0
            next_distance_deviation = step_info['distance_deviation']
            ...
            dyn_adv[ts] = sanitize_advantage(dynamic_collision_advantage)
            sta_adv[ts] = sanitize_advantage(static_collision_advantage)
            dist_adv[ts] = sanitize_advantage(distance_deviation_advantage)
            ang_adv[ts] = sanitize_advantage(angle_deviation_advantage)
            progress_adv[ts] = sanitize_advantage(progress_advantage)
            ...
            progress_advantage = np.array(0.)
            dyn_adv[ts] = float(dynamic_collision_advantage)
            sta_adv[ts] = float(static_collision_advantage)
            dist_adv[ts] = float(distance_deviation_advantage)
            ang_adv[ts] = float(angle_deviation_advantage)
            progress_adv[ts] = float(progress_advantage)
    ...
    progress_list = []
    for ts in clip_data['timestamp_list']:
        da = dyn_adv.get(ts, 0.0)
        sa = sta_adv.get(ts, 0.0)
        di = dist_adv.get(ts, 0.0)
        an = ang_adv.get(ts, 0.0)
        pr = progress_adv.get(ts, 0.0)
        combined_list.append(float(da + sa + di + an + pr))
        dyn_list.append(float(da))
        sta_list.append(float(sa))
        dist_list.append(float(di))
        ang_list.append(float(an))
        progress_list.append(float(pr))
